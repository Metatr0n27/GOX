...
import base64, hmac, json, os, sqlite3, time, uuid
...
AUTH_USER=os.getenv("GOX_AUTH_USER","gox"); AUTH_PASSWORD=os.getenv("GOX_AUTH_PASSWORD","")
LOOPBACK={"127.0.0.1","localhost","::1"}
if HOST not in LOOPBACK and not AUTH_PASSWORD:
    raise RuntimeError("Refusing non-loopback bind without GOX_AUTH_PASSWORD")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def require_auth(self):
        ...
        self.send_json({'error':'authentication required'},401,{'WWW-Authenticate':'Basic realm="GOX Chat Dev"'})
        return False
    def do_GET(self):
        ...
        if self.path=='/api/status': return self.send_json({'interface':'running','persistence':'running','job_queue':'running','agent_bridge':'allowlisted','auth':'enabled' if AUTH_PASSWORD else 'loopback-only','jobs':counts(),'revenue':{'current':0,'target':750}})
