...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path=='/api/status':
            oq=read_queue(); return self.send_json({'interface':'running','persistence':'running','job_queue':'running','agent_bridge':'allowlisted','auth':'enabled' if AUTH_PASSWORD else 'loopback-only','jobs':counts(),'opportunities':oq.get('count',0),'revenue':{'current':0,'target':500}})
